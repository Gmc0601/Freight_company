...
@interface LXZoomingScrollView ()<UIActionSheetDelegate>
...
- (id)initWithFrame:(CGRect)frame
{
    if ((self = [super initWithFrame:frame])) {
        ...
        if (YES) {
            UILongPressGestureRecognizer * longPressGr = [[UILongPressGestureRecognizer alloc] initWithTarget:self action:@selector(longPressToTap:)];
...
- (void) lx_setImageWithURL:(NSURL *)url placeholderImage:(UIImage *)placeholderImage errorImage:(UIImage *)errorImage {
    ...
    [_imageView sd_setImageWithPreviousCachedImageWithURL:url placeholderImage:placeholderImage options:SDWebImageLowPriority | SDWebImageContinueInBackground progress:^(NSInteger receivedSize, NSInteger expectedSize, NSURL * _Nullable targetURL) {
...
#pragma mark 长按
- (void)longPressToTap:(UITapGestureRecognizer *)tap{
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex{
    if (buttonIndex == 0) {
        ...
        [self zoomToRect:CGRectMake(self.imageView.centerX, self.imageView.centerY, self.maximumZoomScale, self.maximumZoomScale) animated:YES];
    }
}
...
- (void)image:(UIImage *) image didFinishSavingWithError: (NSError *) error contextInfo: (void *) contextInfo
{
    ...
    [ConfigModel mbProgressHUD:msg andView:self];
}
